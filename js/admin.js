    ...
    orderCard.innerHTML = `
        <div class="flex flex-wrap justify-between items-center border-b pb-3 mb-3">
            <div>
                ...
                <p class="text-sm text-gray-500">Cliente Chat ID: ${order.chat_id}</p>
                <p class="text-sm text-gray-500">Fecha: ${new Date(order.date).toLocaleString('es-ES')}</p>
            </div>
            <div class="mt-2 sm:mt-0">
                <span class="text-sm font-semibold px-3 py-1 rounded-full ${getStatusColor(order.status)}">
                    ${order.status}
                </span>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p class="font-bold text-right mt-2">Total: $${orderTotal.toFixed(2)}</p>
            </div>
            <div>
                <h3 class="font-bold mb-2">Datos de Entrega:</h3>
                <p class="text-sm text-gray-700"><strong>Dirección:</strong> ${order.address}</p>
                <p class="text-sm text-gray-700"><strong>Método Pago:</strong> ${order.paymentMethod}</p>
            </div>
        </div>
        <div class="mt-4 pt-4 border-t">
            <label for="status-${order.id}" class="block text-sm font-medium text-gray-700 mb-1">Actualizar Estado:</label>
            <div class="flex items-center space-x-2">
                ...
                <button class="btn-update-status bg-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700" data-order-id="${order.id}">
                    Guardar
                </button>
            </div>
        </div>
